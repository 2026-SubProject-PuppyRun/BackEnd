...
cd $DEPLOY_DIRECTORY && \
RELEASE_TAG=$RELEASE_TAG DEPLOY_IMAGE_TAG=$DEPLOY_IMAGE_TAG AWS_S3_BUCKET=$AWS_S3_BUCKET AWS_REGION=$AWS_REGION AWS_ACCOUNT_ID=$AWS_ACCOUNT_ID bash deploy.sh
EOF
)
